...
> 这是历史教程副本，保留当时的实验过程和结论。要运行当前版本，请以
> `training/docs/教程/旅行助手后训练实战教程.md` 和 `training/scripts/run_pipeline.py` 为准。
...
正确做法是保留原请求和 `record_id`，只刷新 `PlannerContext`。当前新建评估集统一使用
`training/scripts/run_pipeline.py --stage eval-data`；本节记录的历史重建结果作为快照保留，
不再提供旧版上下文重建命令。
...
```bash
mkdir -p training/data/planner/sft_runs/260831_example

nohup .venv-training-py311/bin/python3 -u training/scripts/run_pipeline.py \
  --stage sft-data \
  --count 100 \
  --start-index 0 \
  --request-source controlled \
  ...
  --workers 2 \
  --resume \
  --sft-dir training/data/planner/sft_runs/260831_example \
  > training/data/planner/sft_runs/260831_example/pipeline.log 2>&1 &
```
...
当前训练统一由 `training/scripts/run_pipeline.py --stage train` 调度。确认配置和输入后，
可以使用下面的命令启动 LLaMA-Factory：
...
.venv-training-py311/bin/python3 training/scripts/run_pipeline.py \
  ...
  --train-dataset <dataset_name>_train \
  --train-eval-dataset <dataset_name>_val \
...
注意不能用 eval / eval_hard 做训练 prompt。当前统一入口会依次构建 prompt、采样候选、
选择 winner，并导出 SFT / DPO 数据：
...
.venv-training-py311/bin/python3 training/scripts/run_pipeline.py \
  ...
  --eval-dir training/outputs/eval/by_model/sft_qwen25_7b_planner_example \
  --workers 10 \
  --temperature 0.2 \
  --max-tokens 8192 \
  --resume

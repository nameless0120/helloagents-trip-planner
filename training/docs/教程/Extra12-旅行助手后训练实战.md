...
> 这篇文章保留实验过程和历史结论。要运行当前版本，请以同目录的《旅行助手后训练实战教程》以及
> `training/scripts/run_pipeline.py` 为准。

> **文中标记**
>
> - **历史过程**：保留旧版的章节顺序、实验数据、报告路径和结论，方便复盘。
> - **已废弃（仅历史参考）**：旧脚本、日期化启动命令和旧配置字段不要直接执行。
> - **当前替代**：实际运行时，按同目录的《旅行助手后训练实战教程》使用 `run_pipeline.py`。
...
这篇文章继续按这条旧版主线讲；为了让当前代码可以直接运行，执行命令统一收口到当前教程里的分阶段入口。
...
这套 frozen eval 后来一直用来选模型。严格论文口径下，它更像 validation set，不是 blind test。但这个项目本来就是工程玩具，只要训练数据不和评测 prompt 重叠，公开写成“固定评测集上的阶段评估”就够了。
...
> **已废弃：旧版数据生成入口**
>
> 旧版 `training/scripts/eval/generate_sft_data.py` 已移动并废弃，曾经把生成、审计和导出写在多个独立脚本或日期化 shell 里的方式也只保留在历史记录中。当前对应关系是：`sft-request` 检查请求，`sft-context` 检查上下文，`sft-data` 生成 records，`sft-audit` 完成审计、分类、导出和校验。
...
> **已废弃：旧版训练启动脚本**
>
> 旧版 `training/scripts/planner/training/run_*.sh` 和固定实验目录名只代表当时的训练机器与参数，当前仓库不再把它们作为入口。现在用《旅行助手后训练实战教程》里的 `run_pipeline.py --stage train`，通过 `--config`、`--train-dataset` 和 `--train-output-dir` 指定本轮训练。
...
这次用的是 `Qwen2.5-7B-Instruct`，不是 base 模型。配置也尽量沿着主线走：第一版 clean SFT 数据，`cutoff_len=24576`，6 epoch，`lr=1e-6`，FSDP2 + CP=2，6 张 40GB 卡跑。训练能跑通，但成本马上就上来了：一炉大约 7 小时，保存出来的模型 28GB。和一个 LoRA adapter 比，这已经不是同一个迭代手感了。
...
![全参 SFT 与 LoRA 对比图](./images/旅行助手后训练实战/18-全参SFT与LoRA对比图.png)
...
> **当前仍保留：Best-of-N**
>
> Best-of-N Replay 仍然用于从多个候选中挑选训练样本，产品侧多候选 Rerank 也仍然有效。旧版日期化实验脚本已废弃，当前训练数据增强使用 `run_pipeline.py --stage bestofn`；产品侧逻辑在 `backend/app/planner/rerank.py`。
...
> **已废弃：旧版 DPO 启动方式**
>
> 旧版 DPO 的多条独立命令、固定日期目录和旧配置快照只用于解释历史实验。当前用 `run_pipeline.py --stage dpo` 生成并审计 pair，再按需要用 `run_pipeline.py --stage train` 启动 LLaMA-Factory 训练。
...
> **已废弃：历史评测命令**
>
> 本章里的日期、checkpoint 和指标是历史实验记录，不是当前默认模型。要重新跑评测，使用 `run_pipeline.py --stage eval`，并把模型服务地址、模型名和输出目录替换成当前这一轮的值。

...
> **当前入口**
>
> 本文尽量保留旧版的章节顺序和实验记录。实际运行时，从项目根目录执行 `training/scripts/run_pipeline.py`；当前入口把请求检查、上下文检查、SFT 数据生成、审计、Best-of-N、DPO、训练和评测拆成独立阶段。
>
> **文中标记**
>
> - **当前入口**：可以直接执行，优先使用 `run_pipeline.py`。
> - **已废弃（仅历史参考）**：旧路径、日期化 shell 脚本、旧阶段名和旧配置写法只用于解释当时的实验，不要照着执行。
>
> 历史数据、报告、指标和 checkpoint 名保留用于复盘；它们不等于当前推荐命令。
...
- [9.8 当前代码排查：`party` 字段](#98-当前代码排查party-字段)
- [9.9 LLaMA-Factory 本地改动](#99-llama-factory-本地改动)
...
这个检查不会调用 API，也不会启动 GPU。确认环境后，再按后面的旧版主线阅读和执行；可执行命令都已经换成当前阶段名。
...
> **已废弃：旧版固定复现实验配置**
>
> 下面的 YAML 保留旧版实验的参数口径，里面的 adapter、数据集和输出路径都是当时的固定路径，当前不能直接照抄。当前训练配置和数据集名称请以本轮 `sft-audit` 输出及 `run_pipeline.py --stage train` 的参数为准。
...
旧版启动时会显式指定 2 张 40GB 卡；当前训练命令如下：

> **已废弃：旧版训练启动脚本**
>
> `training/scripts/planner/training/run_260513_bestofn1200_replay_from_final.sh` 只代表历史实验的机器和参数，当前仓库不再把它作为入口。
>
> 当前训练使用总入口，并把本轮审计后登记的数据集传给 `train` 阶段：
>
> ```bash
> .venv-training-py311/bin/python3 training/scripts/run_pipeline.py \
>   --stage train \
...
这条当前命令要在第 4 章完成数据生成和第 5 章完成审计后执行；`${SFT_DATASET}` 是本轮 `sft-audit` 使用的 dataset prefix。
...
> **旧版底层脚本写法**
>
> 下面的 `build_eval_set.py` 仍可用于定位单个步骤，但当前推荐入口是 `run_pipeline.py --stage eval-data`。需要从统一入口开始时，使用第 2.3 节的当前命令。
...
正确做法仍然是保留原请求和 `record_id`，只刷新 `PlannerContext`。但旧版的 `rebuild_eval_contexts.py` 已从当前仓库移除。

> **已废弃：旧版上下文重建脚本**
>
> `training/scripts/planner/eval/rebuild_eval_contexts.py` 和对应的日期化重建命令只保留在历史记录里，当前不要执行。当前需要新建冻结评测集时，使用 `run_pipeline.py --stage eval-data`；如果只是使用已有评测集，直接读取 `training/data/planner/eval/records.jsonl` 或 `training/data/planner/eval_hard/records.jsonl`。
>
> ```bash
> .venv-training-py311/bin/python3 training/scripts/run_pipeline.py \
>   --stage eval-data \
...
>   --date-mode mixed \
>   --difficulty standard \
>   --eval-data-dir training/data/planner/eval_reader
> ```
...
> **已废弃：旧版一体化 SFT 阶段**
>
> 旧版命令使用 `--stage sft`，当前阶段名已经拆开；不要继续执行这个旧阶段。
>
> 当前先生成 records：
>
> ```bash
...
audit_budget/           # 预算贴合审计
classification/         # 可用性分类
export_budget_clean/     # 导出的预算干净子集
llm_usage.jsonl          # 强模型 usage 记录
pipeline_manifest.json   # 总入口参数
```
...
> **已废弃：旧版配置字段示例**
>
> `lora_rank / lora_alpha / gradient_accumulation_steps` 是旧版 LLaMA-Factory YAML 的写法，旧配置快照只用于复盘。当前请以仓库里的 `training/configs/qwen25_7b/*.yaml` 和 `run_pipeline.py --stage train` 的参数为准。
...
旧版训练脚本会等待 GPU 空闲后启动 LLaMA-Factory；当前训练改由总入口直接调度：

> **已废弃：旧版日期化训练命令**
>
> 下面的固定脚本只用于对照当时的实验记录。当前训练仍使用 `run_pipeline.py --stage train`，不要依赖固定日期目录或机器编号。
>
> ```bash
> .venv-training-py311/bin/python3 training/scripts/run_pipeline.py \
>   --stage train \
...
> **当前仍保留：Best-of-N**
>
> Best-of-N Replay 仍然是当前训练数据增强流程，产品侧多候选 Rerank 也仍然保留。旧版日期化脚本已废弃，当前使用 `run_pipeline.py --stage bestofn`。
...
这个入口会依次完成 prompt 构建、候选采样和 winner 导出。它要求先启动 SFT Planner 服务，输出目录中的 `prompts.jsonl`、`candidates.jsonl`、`selected.jsonl` 和 LLaMA-Factory 文件会保留下来；需要定位单个步骤时，再直接运行对应底层脚本。

### 7.2 采样候选

`run_pipeline.py --stage bestofn` 会在同一轮内完成多温度采样。`--bestofn-base-url` 和 `--bestofn-api-model` 用来指定本地 Planner 服务。

### 7.3 选择 winner 并导出训练数据

同一轮的最后一步由总入口调用 `select_best.py`，会同时导出 SFT 和 DPO 格式。导出后总入口会执行 `TripPlan` 格式校验；如果没有可用的 SFT train，流水线会停止。
...
> **已废弃：旧版 DPO 多脚本串联方式**
>
> 旧版的固定日期命令和独立脚本组合只用于说明实验过程。当前用 `run_pipeline.py --stage dpo` 生成并审计 pair，再按需要用 `run_pipeline.py --stage train` 训练。
...
> **当前入口：统一评测阶段**
>
> 本节保留旧版 standard / hard 的比较方法。旧版直接调用 `training/scripts/eval/eval_pipeline.py` 的命令只作为底层调试参考；当前建议统一使用 `run_pipeline.py --stage eval`，并为每轮填写实际的模型服务地址和输出目录。
...
先启动模型服务，然后跑 standard eval。下面保留旧版直接调用评测子脚本的写法，便于对照历史报告：
...
当前请求 schema 在 `backend/app/models/schemas.py`，`PlannerContext` 组装在 `backend/app/planner/`，SFT 数据入口在 `training/scripts/planner/data/generate_sft_data.py`。
...
旧版请求生成流程可能只得到 `{"total": 2}` 这样的部分对象，当前生成入口会在送入后续校验前补齐人数分项。遇到 `party` 校验错误时，先确认当前命令调用的是 `planner/data/generate_sft_data.py`，再查看对应 run 下的 `records.jsonl`、`errors.jsonl` 和 `pipeline_manifest.json`。
...
这里的 `--dry-run` 只检查子命令，不访问高德。需要实际验证 PlannerContext 时，去掉 `--dry-run`，并配置高德 API Key；`--dry-run-context` 仍是底层生成脚本支持的当前参数。

## 9.9 LLaMA-Factory 本地改动
